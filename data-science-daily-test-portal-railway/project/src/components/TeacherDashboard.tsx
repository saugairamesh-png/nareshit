...
export default function TeacherDashboard({ onLogout }: TeacherDashboardProps) {
  ...
  const [isImportingQuestionFile, setIsImportingQuestionFile] = useState(false);
  ...
  // Shared cleaner/merger used by both the paste-JSON box and the file upload path.
  const mergeParsedQuestionsIntoForm = (parsed: any): { mcqCount: number; codingCount: number } | null => {
    ...
    return { mcqCount: cleanedMcqs.length, codingCount: cleanedCoding.length };
  };
  ...
  const handleBulkQuestionImportSubmit = () => {
    ...
    const result = mergeParsedQuestionsIntoForm(parsed);
    if (!result) return;
  ...
  const handleQuestionFileUpload = (file: File) => {
    setBulkQuestionImportError(null);
    const ext = file.name.split(".").pop()?.toLowerCase() || "";
    if (!["json", "csv", "docx", "doc", "pdf"].includes(ext)) {
      setBulkQuestionImportError("Unsupported file type. Please upload a .json, .csv, .docx, or .pdf file.");
      return;
    }

    setIsImportingQuestionFile(true);
    const reader = new FileReader();
    reader.onload = async () => {
      try {
        const fileBase64 = typeof reader.result === "string" ? reader.result : "";
        const res = await fetch("/api/teacher/parse-question-file", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
        ...
        const data = await res.json();
        if (!res.ok) {
          setBulkQuestionImportError(data?.error || "Failed to process the uploaded file.");
          return;
        }
        const result = mergeParsedQuestionsIntoForm(data);
        if (result) {
          setShowBulkQuestionImport(false);
        }
      } catch (err) {
        setBulkQuestionImportError("Something went wrong while uploading or parsing this file. Please try again.");
      } finally {
        setIsImportingQuestionFile(false);
      }
    };
    reader.onerror = () => {
      setBulkQuestionImportError("Could not read the selected file.");
      setIsImportingQuestionFile(false);
    };
                ...
                >
                  <div>
                    <div className="text-xs font-black text-emerald-950 uppercase tracking-wide flex items-center gap-1.5">
                      <span>📋 Bulk Import Questions (JSON, CSV, Word, PDF)</span>
                    </div>
                    <div className="text-[10px] text-emerald-700 font-medium">Upload a file or paste JSON to add many MCQs and coding challenges at once instead of adding them one by one.</div>
                  </div>
                ...
                {showBulkQuestionImport && (
                  <div className="space-y-3 pt-1">
                    ...
                    <div className="bg-white border border-emerald-300 rounded-lg p-3 space-y-1.5">
                      <label className="text-[10px] font-bold text-emerald-900 uppercase tracking-wide block">
                        Upload a file
                      </label>
                      <p className="text-[9.5px] text-emerald-700">
                        Accepts <strong>.csv</strong> (columns: type, questionText, optionA-D, correctOption, explanation, starterCode, expectedKeywords), <strong>.docx</strong>, <strong>.pdf</strong>, or <strong>.json</strong>. Word/PDF documents are read automatically and structured by AI.
                      </p>
                      <input
                        type="file"
                        accept=".json,.csv,.docx,.doc,.pdf"
                        disabled={isImportingQuestionFile}
                        onChange={(e) => {
                          const file = e.target.files?.[0];
                          if (file) handleQuestionFileUpload(file);
                          e.target.value = "";
                        }}
                        className="w-full text-[10.5px] text-emerald-900 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-[10px] file:font-bold file:uppercase file:bg-emerald-600 file:text-white hover:file:bg-emerald-700 file:cursor-pointer cursor-pointer disabled:opacity-50"
                      />
                      {isImportingQuestionFile && (
                        <p className="text-[10.5px] text-emerald-700 font-semibold flex items-center gap-1.5">
                          <span className="w-3 h-3 border-2 border-emerald-600 border-t-transparent rounded-full animate-spin inline-block" />
                          Reading and structuring questions from your file...
                        </p>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-[9px] text-emerald-600 font-bold uppercase">or paste JSON</span>
                      <div className="h-px bg-emerald-200 flex-1" />
                    </div>
              ...
              {selectedInterview.videoUrl && (
                <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 text-slate-100 space-y-2.5">
                  ...
                  <a
                    ...
                    download={`interview-recording-${selectedInterview.studentName || selectedInterview.id}.webm`}
                    className="flex items-center justify-center gap-1.5 w-full bg-indigo-600 hover:bg-indigo-500 text-white font-mono font-bold text-[10px] uppercase tracking-wider py-2 rounded-lg transition cursor-pointer"
                  >
