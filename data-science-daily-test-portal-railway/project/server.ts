...
// 5b. Bulk import: parse an uploaded question file (JSON / CSV / DOCX / PDF) into
// the standard { mcqs, coding } schema used by the Weekly/Monthly Mock Test scheduler.
app.post("/api/teacher/parse-question-file", async (req, res) => {
  const { filename, fileBase64 } = req.body;
  if (!filename || !fileBase64) {
    res.status(400).json({ error: "Both filename and fileBase64 are required." });
    return;
  }
  ...
  try {
    ...
    if (ext === "json") {
      ...
      const mcqs = Array.isArray(parsed) ? parsed : Array.isArray(parsed?.mcqs) ? parsed.mcqs : [];
      const coding = Array.isArray(parsed?.coding) ? parsed.coding : [];
      res.json({ mcqs, coding });
      return;
    }

    // ---- CSV: parse directly using expected column headers, no AI needed ----
    if (ext === "csv") {
      const text = buffer.toString("utf-8");
      const rows = text.split(/\r?\n/).filter(r => r.trim().length > 0);
      if (rows.length < 2) {
        res.status(400).json({ error: "CSV file appears to be empty or missing a header row." });
        return;
      }
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const type = (rec["type"] || "mcq").toLowerCase();
        if (type === "coding") {
          if (rec["questiontext"]) {
            coding.push({
              questionText: rec["questiontext"],
              starterCode: rec["startercode"] || "",
              expectedKeywords: rec["expectedkeywords"] || ""
            });
          }
        } else {
          if (rec["questiontext"] && rec["optiona"]) {
            mcqs.push({
              questionText: rec["questiontext"],
              options: [rec["optiona"] || "", rec["optionb"] || "", rec["optionc"] || "", rec["optiond"] || ""],
              correctOption: Number.isInteger(parseInt(rec["correctoption"], 10)) ? parseInt(rec["correctoption"], 10) : 0,
    ...
    // ---- DOCX / PDF: extract raw text, then ask Gemini to structure it ----
    let extractedText = "";
    if (ext === "docx" || ext === "doc") {
      ...
      res.status(400).json({ error: `Unsupported file type ".${ext}". Please upload a .json, .csv, .docx, or .pdf file.` });
      return;
    }
    ...
    const ai = getAi();
    if (!ai) {
      res.status(503).json({ error: "Gemini API key is not configured, so PDF/Word files can't be auto-structured into questions. CSV and JSON uploads still work without it." });
      return;
    }

    const prompt = `
You are an expert Data Science and AI educator reviewing a document of exam questions supplied by an instructor.

---- BEGIN DOCUMENT TEXT ----
${extractedText.slice(0, 15000)}
---- END DOCUMENT TEXT ----

Extract every multiple choice question (MCQ) and coding challenge you can find in this document, and return them as a single JSON object in EXACTLY this structure (no markdown, no commentary):
{
  "mcqs": [
    { "questionText": "...", "options": ["...", "...", "...", "..."], "correctOption": 0, "explanation": "..." }
  ],
  "coding": [
    { "questionText": "...", "starterCode": "def solution():\\n    pass", "expectedKeywords": "def,return" }
  ]
}

Rules:
- If the document doesn't clearly state the correct answer for an MCQ, make your best expert judgment and note it briefly in "explanation".
- If the document has no coding challenges, return an empty "coding" array. Same for "mcqs" if none are found.
- Do not invent unrelated questions — only extract/rephrase what's actually present in the document.
...
`;
    ...
    console.error("[Express Backend] Failed to parse uploaded question file:", error);
    res.status(500).json({ error: error.message || "Failed to read or structure questions from the uploaded file." });
  }
});
